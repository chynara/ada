...
# C8 данных Вопрос - Dataset README
В этом документе содержатся предположения и определения для отчетов займа объема.

## Источник данных
Общие Возникновение и Disbursement системы (ХПК)

## Расписание Обновить данные
Данные будут обновляться каждый квартал в течение по крайней мере семи предыдущих кварталов для учета корректировок. После периода корректировки, данные будут окончательными.

## Предположения
1. Кредиты включаются в столбцы ПОЛУчАТЕЛИ, # КРЕДИТОВ Originated, $ КРЕДИТОВ Originated, # освоенных средств и $ освоенных средств, если срок даты начала кредита подпадает под награду года сообщила о таблице и первой выплаты что кредит падает в течение этого квартала. Выплаты, возникшие до года премии учитываются в 1 квартале и Выплаты, возникающие после окончания года премии учитываются в квартал 4. Для данных Year-To-Date, все числа являются накопительными.
1. Выплаты включаются в колонках # освоенных средств и $ выплат, если кредит был включен в подсчетов предоставлении ссуд и даты выплатах были меньше или равно последний день квартала сообщили об этой таблице. За четвертый квартал отчетов это общее число выплат по состоянию на дату отчета, созданного для того, чтобы включить любые просроченные выплаты. Выплаты на втором, третьем и четвертом кварталах являются нарастающим итогом с начала года премии.
1. Консолидация займов не включены в любом запросе
1. по докладу, только если по крайней мере, один кредит возник в том, что школа имеет право для включения в доклад Школа появляется.
1. Кредиты с текущим статусом кредита погашенных исключены из обоих запросов
1. Выплаты в истории с количеством нуля, не учитываются в количестве выделяемых средств.
